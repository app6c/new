// Script para popular o banco de dados com os padrões emocionais
...
// Função para criar um padrão emocional via API
async function createEmotionalPattern(pattern) {
  const { patternType, areaType, isPain, description } = pattern;
  ...
  const curl = `curl -X POST http://localhost:5000/api/emotional-patterns \
    -H "Content-Type: application/json" \
    -d '${data.replace(/'/g, "\\'")}'`;
  
  try {
    console.log(`Criando padrão: ${patternType} - ${areaType} - ${isPain ? 'Dor' : 'Recurso'}`);
    execSync(curl);
    console.log('Criado com sucesso!\n');
  } catch (error) {
    console.error(`Erro ao criar padrão: ${patternType} - ${areaType} - ${isPain ? 'Dor' : 'Recurso'}`);
    console.error(error.toString());
  }
}

// Função principal para executar a criação de todos os padrões
async function seedEmotionalPatterns() {
  console.log('Iniciando a criação dos padrões emocionais...\n');
  
  // Converter os dados do JSON em um array de padrões
  const patterns = [];
  
  for (const [patternType, areas] of Object.entries(patternsData)) {
    for (const [areaType, types] of Object.entries(areas)) {
      // Adicionar o padrão de dor
      patterns.push({
        patternType,
        areaType,
        isPain: true,
        description: types.Dor
      });
      
      // Adicionar o padrão de recurso
      patterns.push({
        patternType,
        areaType,
        isPain: false,
        description: types.Recurso
      });
    }
  }
  
  // Criar cada padrão emocional
  for (const pattern of patterns) {
    await createEmotionalPattern(pattern);
  }
  
  console.log('Criação de padrões emocionais concluída!');
}

// Executar a função principal
seedEmotionalPatterns();